...
from models.voxel import VoxelFingerprintNet
from data_util import FragmentDataset, SharedFragmentDataset, FingerprintDataset, LIG_TYPER,\
    REC_TYPER
from grid_util import get_batch
from metrics import mse, bce, tanimoto, cos, top_k_acc,\
    ...
    def train(self, save_path=None, custom_steps=None, checkpoint_callback=None, data=None):
        ...
        for epoch in range(self._args['num_epochs']):
            ...
            for step in train_pbar:
                ...
                mols = [example['mol_info'] for example in examples]
                
                c_affinities = []
                for i in mols:
                    a = affinity(i[0], i[1], i[2])
                    p_affinities.append(a)


                predicted_fp = self._models['voxel'](torch_grid)

                p_affinities = pred_affinity(mols, predicted_fp)
