    ...
    def __init__(self, args, device='cuda', with_log=True):
        ...
        if 'moad_partition' in self._args:
            if self._args['moad_partition'] == 'moad_partitions':
                ...
                from config import moad_400 as mp
            else:
                from config import moad_partitions as mp
        else:
            from config import moad_partitions as mp        
...
class VoxelNet(LeadoptModel):
    ...
    def load_data(self):
        ...
        train_dat = SharedFragmentDataset(
            dat,
            filter_rec=set(get_bios(mp.TRAIN)),
            filter_smi=set(mp.TRAIN_SMI),
        ...
        val_dat = SharedFragmentDataset(
            dat,
            filter_rec=set(get_bios(mp.VAL)),
            filter_smi=set(mp.VAL_SMI),
            fdist_min=self._args['fdist_min'],
            fdist_max=self._args['fdist_max'],
            fmass_min=self._args['fmass_min'],
            fmass_max=self._args['fmass_max'],
    ...
    def run_test(self, save_path, use_val=False):
        ...
        test_dat = FragmentDataset(
            ...
            filter_rec=set(get_bios(mp.VAL if use_val else mp.TEST)),
            filter_smi=set(mp.VAL_SMI if use_val else mp.TEST_SMI),
            fdist_min=self._args['fdist_min'],
            fdist_max=self._args['fdist_max'],
            fmass_min=self._args['fmass_min'],
            fmass_max=self._args['fmass_max'],
        ...
        train_smiles = train_dat.get_valid_smiles()
        val_smiles = val_dat.get_valid_smiles()
        test_smiles = [test_dat[i]['smiles'] for i in range(len(test_dat))]
        all_smiles = list(set(train_smiles + val_smiles + test_smiles))

        all_fp = fingerprints.for_smiles(all_smiles).numpy()
        ...
        with torch.no_grad():
            for i in pbar:
                ...
                all_counter = 0
                c_counter = 0
                for k in range(len(predicted_fp)):
                    pfp = predicted_fp[k]
                    dist_fn = DIST_FN[self._args['dist_fn']]
                    dist = dist_fn(
                        torch.Tensor(pfp).unsqueeze(0),
